...
func main() {
	...
	for !currentDate.After(endDate) {
		...
		partitionName := fmt.Sprintf("metrics_%s", currentDate.Format("20060102"))

		query := fmt.Sprintf(`
			CREATE TABLE IF NOT EXISTS %s PARTITION OF metrics
			FOR VALUES FROM ('%s') TO ('%s');
		`, partitionName, currentDate.Format("2006-01-02"), nextDate.Format("2006-01-02"))
	...
	log.Printf("Created partitions from %s to %s", startDate.Format("2006-01-02"), endDate.Format("2006-01-02"))
}
