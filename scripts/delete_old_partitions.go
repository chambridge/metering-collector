...
func main() {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&partitionName); err != nil {
			log.Printf("Failed to scan partition name: %v", err)
			continue
		}

		// Extract date from partition name (e.g., metrics_20250527)
		if !strings.HasPrefix(partitionName, "metrics_") {
			log.Printf("Skipping invalid partition name: %s", partitionName)
			continue
		}

		dateStr := strings.TrimPrefix(partitionName, "metrics_")
		if len(dateStr) != 8 {
	...
	if err := rows.Err(); err != nil {
		log.Fatalf("Error iterating partitions: %v", err)
	}
	...
	if len(partitionsToDrop) == 0 {
		log.Printf("No partitions older than %s found", cutoffDate.Format("2006-01-02"))
	} else {
		log.Printf("Dropped %d partitions older than %s", len(partitionsToDrop), cutoffDate.Format("2006-01-02"))
	}
}
